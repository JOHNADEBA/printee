...
const AddCoinsForm: React.FC<AddCoinsFormProps> = ({ setError, setSuccess, error, success }) => {   
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        const response = await api.post<{ clientSecret: string }>("/user/create-payment-intent", { amount });
        if (!response.data) {
        ...
        const { paymentIntent, error: stripeError } = await stripe.confirmCardPayment(response.data.clientSecret, {
        ...
        if (paymentIntent?.status === 'succeeded') {
          ...
          if (confirmResponse.data) {
            setSuccess(`Added ${amount} coins! New balance: ${confirmResponse.data.coins}`);
            setAmount('');
           updateUser()
          } else {
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-1">Card Details</label>
          <CardElement options={cardElementOptions} className="p-2 border border-gray-300 rounded" />
        </div>
        <button
          type="submit"
